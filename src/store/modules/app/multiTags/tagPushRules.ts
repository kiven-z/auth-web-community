...
/**
 * 超过 MaxTagsLevel 时移除 index 1 的标签
 */
export function trimAfterPush(tags: RouteConfigs[]): RouteConfigs[] {
  const maxTagsLevel = getConfig()?.MaxTagsLevel;
  if (!maxTagsLevel || !isNumber(maxTagsLevel) || tags.length <= maxTagsLevel) {
    return tags;
  }
  const next = [...tags];
  next.splice(1, 1);
...
export function applyPushTag(tags: RouteConfigs[], tag: TagRouteItem): RouteConfigs[] | null {
  ...
  let next = trimBeforePush(tags, tag);
  next = [...next, tag as RouteConfigs];
  return trimAfterPush(next);
}
