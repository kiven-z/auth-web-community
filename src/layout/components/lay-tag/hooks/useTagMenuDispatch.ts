...
import { TAG_MENU_ACTION_BY_INDEX, TAG_MENU_INDEX, TagMenuAction } from '../constants/tagMenu';
import type { CloseScope } from '../types';
import { computeMenuState, resolveContextMenuState } from '../utils/contextMenuPolicy';
import { applyMenuState, positionContextMenu } from '../utils/menuView';
...
export function useTagMenuDispatch(deps: TagMenuDispatchDeps) {
  ...
  function openMenu(tag: RouteConfigs, event: MouseEvent): void {
    ...
    const { left, top } = positionContextMenu(event, containerDom.value, settingStore.hiddenSideBar);
