...
declare global {
  ...
  interface CustomizeRouteMeta {
    ...
    /** 是否在菜单中显示（默认`true`）`可选` */
    showLink?: boolean;
    /** 登录可见：所有登录用户可见，不校验角色（默认`false`）`可选` */
    ...
    /**
     * 页面级权限码（按钮权限）。
     * 字符串或数组均为 AND：须全部满足；未配置则不校验。
     */
    auths?: string | Array<string>;
    ...
    activePath?: string;
    /**
     * 工作空间侧栏分区键（个人空间 / 用户工作台）。
     * 嵌套页与列表页写同一值即可高亮同一侧栏项。
     */
    section?: string;
    /** 当前页面是否已经加载过 */
    loaded?: boolean;
  ...
  /**
   * 经层级处理后的路由节点（`buildHierarchyTree` 会写入 parentId / pathList）
   */
  type AppRouteRecord = import('vue-router').RouteRecordRaw & {
  ...
  interface RouteConfigsTable {
    ...
    redirect?: string;
    meta?: {
      /** 菜单名称（i18n key 或纯文案；用 key 时须在根目录 `locales` 中补条目）`必填` */
      title: string;
      /** 菜单图标 `可选` */
      icon?: string | FunctionalComponent;
      /** 是否在菜单中显示（默认`true`）`可选` */
