...
describe('session-preferences', () => {
  ...
  it('returns defaults when preferences are not stored', async () => {
    ...
    const { readSessionPreferences } = await import('@/core/session/remember/session-preferences');
  ...
  it('merges partial updates when writing preferences', async () => {
    ...
    const { writeSessionPreferences } = await import('@/core/session/remember/session-preferences');
