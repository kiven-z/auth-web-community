...
vi.mock('@/core/session/remember/session-preferences', () => ({
  readSessionPreferences: mockReadSessionPreferences,
}));

vi.mock('@/core/session/remember/remember-preferences', () => ({
...
describe('accessTokenRefresh', () => {
  ...
  it('runs refresh in single-flight mode under concurrency', async () => {
    ...
    const { refreshAccessTokenSingleFlight } = await import('@/core/auth/access-token-refresh');
  ...
  it('injects refreshed bearer token when replaying request and headers are empty', async () => {
    ...
    const { replayRequestWithRefreshedToken } = await import('@/core/auth/access-token-refresh');
